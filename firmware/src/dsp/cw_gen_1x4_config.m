...
function cw_gen_1x4_config(this_block)

  this_block.setTopLevelLanguage('VHDL');

  this_block.setEntityName('cw_gen_1x4_ip_struct');
  ...
  lo_out_port.setType('UFix_192_0');
  this_block.addSimulinkOutport('lo_out_unscaled');
  lo_out_unscaled_port = this_block.port('lo_out_unscaled');
  lo_out_unscaled_port.setType('UFix_192_0');

  % -----------------------------
  if (this_block.inputTypesKnown)
    % do input type checking, dynamic output type and generic setup in this code block.
    if (this_block.port('phase_step').width ~= 50);
      this_block.setError('Input data type for port "phase_step" must have width=32.');
    end
    if (this_block.port('phase_offset').width ~= 50);
  ...
  %    this_block.addFile('');
  %    this_block.addFile('');
  this_block.addFile('dsp/cw_gen_1x4_ip_struct.vhd');
